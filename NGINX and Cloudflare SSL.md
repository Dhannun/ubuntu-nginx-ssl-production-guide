...
2. Select your domain (e.g., `delqore.ng`)
3. Go to **DNS** → **Records**
4. Click **Add record**
5. Configure:
   - **Type**: A
...
**Result**: Your subdomain will be `your-subdomain.delqore.ng`
...
4. Zone Resources: Include → Specific zone → `delqore.ng`
...
- `your-subdomain.delqore.ng` - your full subdomain
- `your-email@example.com` - your email address
```bash
sudo certbot certonly \
  --dns-cloudflare \
  --dns-cloudflare-credentials /etc/letsencrypt/cloudflare.ini \
  -d your-subdomain.delqore.ng \
...
- `/etc/letsencrypt/live/your-subdomain.delqore.ng/fullchain.pem`
- `/etc/letsencrypt/live/your-subdomain.delqore.ng/privkey.pem`
...
```bash
sudo nano /etc/nginx/sites-available/your-subdomain.delqore.ng
```

Add this configuration (replace `your-subdomain.delqore.ng` and port `9090` with your values):
```nginx
# HTTP - Redirect all traffic to HTTPS
server {
    listen 80;
    listen [::]:80;
    server_name your-subdomain.delqore.ng;
...
server {
    ...
    # SSL Certificate
    ssl_certificate /etc/letsencrypt/live/your-subdomain.delqore.ng/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/your-subdomain.delqore.ng/privkey.pem;
...
```bash
sudo ln -s /etc/nginx/sites-available/your-subdomain.delqore.ng /etc/nginx/sites-enabled/
```
...
```bash
curl -I http://your-subdomain.delqore.ng
```
...
### Test HTTPS:
```bash
curl -I https://your-subdomain.delqore.ng
```
...
Open: `https://your-subdomain.delqore.ng`
...
```bash
dig +short your-subdomain.delqore.ng
```
...
2. Get certificate: `sudo certbot certonly --dns-cloudflare --dns-cloudflare-credentials /etc/letsencrypt/cloudflare.ini -d new-subdomain.delqore.ng`
...
```bash
sudo certbot renew --cert-name your-subdomain.delqore.ng
```

### Delete a certificate:
```bash
sudo certbot delete --cert-name your-subdomain.delqore.ng
```
...
sudo certbot certonly \
  --dns-cloudflare \
  --dns-cloudflare-credentials /etc/letsencrypt/cloudflare.ini \
  -d new-subdomain.delqore.ng \
  --email your-email@example.com \
  --agree-tos

# 3. Create Nginx config
sudo nano /etc/nginx/sites-available/new-subdomain.delqore.ng
# (paste configuration from Step 4 above, update subdomain and port)

# 4. Enable site
sudo ln -s /etc/nginx/sites-available/new-subdomain.delqore.ng /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx

# 5. Test
curl -I https://new-subdomain.delqore.ng
```

---

## Example: Setting Up kano.delqore.ng for Port 9090
```bash
...
sudo certbot certonly \
  ...
  --dns-cloudflare-credentials /etc/letsencrypt/cloudflare.ini \
  -d kano.delqore.ng \
  --email admin@delqore.ng \
  --agree-tos

# 3. Create Nginx config
sudo nano /etc/nginx/sites-available/kano.delqore.ng
```
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/kano.delqore.ng/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/kano.delqore.ng/privkey.pem;
...
```bash
# 4. Enable and reload
sudo ln -s /etc/nginx/sites-available/kano.delqore.ng /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx

# 5. Test
curl -I https://kano.delqore.ng
```

**Done!** Visit `https://kano.delqore.ng` in your browser.
